...
//  SampleTinder-MVVM
//
//  Created by Makoto on 2022/04/23.
//
...
final class RegisterViewModel {
    ...
    init() {
        ...
        let nameValid = nameTextOutput.asObservable().map { text -> Bool in
            
            return text.count >= 5
        }
        
        let emailValid = emailTextOutput.asObservable().map { text -> Bool in
            
            return text.count >= 5
        }
        
        let passwordValid = passwordTextOutput.asObservable().map { text -> Bool in
            
            return text.count >= 5
        }
        
        Observable.combineLatest(nameValid, emailValid, passwordValid) { $0 && $1 && $2 }.subscribe { validAll in
...
extension RegisterViewModel: RegisterViewModelInputs, RegisterViewModelOutputs {
